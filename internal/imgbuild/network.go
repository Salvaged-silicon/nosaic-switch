...
// releaseASICScript runs the release and REPORTS a failure instead of being one.
//
// The release is a oneshot, and an s6 oneshot that exits non-zero takes the
// whole service database down with it -- which on a board whose chip cannot be
// released yet means the switch boots to a rescue shell: no network, no login,
// nothing to debug with. That is strictly worse than a switch that comes up and
// says its datapath is dead, and it is exactly the state a board under bring-up
// is in. Observed on the 7150S-52, where releasing the reset genuinely does not
// bring the chip onto the bus yet.
//
// ⚠ THIS DOES NOT WEAKEN A/B ROLLBACK, AND THAT IS WHY IT IS SAFE.
//
// The signal that an image is broken is nosd exiting non-zero, which it does
// when it cannot find its chip -- trial-confirm sees that and rolls back. This
// oneshot failing was a second, redundant copy of the same signal, and the
// redundant copy is the one that costs an operator their console.
const releaseASIC = `#!/bin/sh
# Generated by nosaic.
say() { echo "NOSAIC-ASIC $*"; }

if nosaic platform release-asic; then
    say "released"
    exit 0
fi

say "RELEASE FAILED -- the switch chip is not on the bus"
say "the system is coming up anyway so that it can be reached and looked at;"
say "nosd will fail for the same reason, which is what fails an A/B trial"
exit 0
`
