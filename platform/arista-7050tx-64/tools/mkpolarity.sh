...
# Families that need the SURVIVAL rule but not the collapse.
#
# Polarity above is per LANE, so four 10G lanes fold into one 40G bitmask. Lane
# maps and the SerDes firmware mode are per MACRO already: the capture carries
# one entry per four-lane group, so there is nothing to fold -- what they need
# is only the rule that decides which ports still exist once the QSFP cages
# stop being four 10G ports each.
#
# ⚠ SAME RULE, DIFFERENT ARITHMETIC, AND FOLDING THESE WOULD CORRUPT THEM.
# A lane map is a permutation written as four nibbles (0x1302 means physical
# lane order 1,3,0,2). Or-ing four of those together produces a number that is
...
SURVIVOR_FILTER='
{
    ...
    name = substr(line, 1, eq - 1)
    dot  = length(name); while (dot > 0 && substr(name, dot, 1) != ".") dot--
    stem = (dot > 0) ? substr(name, 1, dot - 1) : name
    u    = length(stem); while (u > 0 && substr(stem, u, 1) != "_") u--
    port = substr(stem, u + 1) + 0

    # Below the cages every port survives. At and above them, only the first
    # lane of each group is still a port once the cage runs as one 40G.
    if (port <= 48 || breakout == "1") { print line; next }
    if ((port - 49) % 4 == 0) print line
}'
...
# The rest of the SerDes wiring, which needs no folding.
#
# ⚠ WITHOUT THE LANE MAPS A 40G LINK COMES UP AND CARRIES NOTHING. The PCB does
...
# way, so the link trains, the cage reports up at 40000, and every status the
# SDK offers says the port is healthy -- these were missing on this board and
# all three cabled cages received exactly zero frames.
serdes=$(printf '%s\n' "$src" |
	sed 's/^[[:space:]]*//' |
	grep -E "^(xgxs_(rx|tx)_lane_map|serdes_firmware_mode)_[0-9]+(\.[0-9]+)?=" |
	sed "s/\.${UNIT_FROM}=/.${UNIT_TO}=/" |
	awk -v breakout="$QSFP_BREAKOUT" "$SURVIVOR_FILTER" |
	sort)
props=$(printf '%s\n%s\n' "$props" "$serdes")
...
# The lane maps are counted separately because their absence is silent in a way
# the polarity count is not: a board with none of them still produces a file
# that looks complete.
lm=$(printf '%s\n' "$props" | grep -c '^xgxs_' || true)
fw=$(printf '%s\n' "$props" | grep -c '^serdes_firmware_mode_' || true)
echo "$0: emitted $lm SerDes lane map entries and $fw firmware modes" >&2
[ "$lm" -gt 0 ] || echo "$0: WARNING: no lane maps -- 40G cages will link and pass nothing" >&2
