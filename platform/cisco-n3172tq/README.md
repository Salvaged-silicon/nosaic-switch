...
| Kernel config | [`recipes/linux/config/x86_64.fragment`](../../recipes/linux/config/x86_64.fragment) | three symbols added, one of them unresolved |
...
| Management | one of four PCH GbE ports; **which one is unresolved** |
| Boot | **UEFI firmware directly** → EDK2 shell → our `BOOTX64.EFI`. No vendor bootloader in the path |
| Console | `ttyS0` @ **9600** |
...
> **Read [todo.md](docs/todo.md) before powering this switch on and walking
> away.** NOSaic declares no platform HAL for this board, so it does not drive
> the fans, and nobody has established what this chassis does with its fans
> when no software is driving them.
