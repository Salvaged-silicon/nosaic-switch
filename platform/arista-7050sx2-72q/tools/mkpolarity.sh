...
printf '# Each flip is emitted twice: keyed by port name (xeN, which is what\n'
printf '# the SDK looks up first) and by logical port number. Only the first\n'
printf '# is in the SDK lookup chain; the second is there to be readable.\n#\n' 
...
while read -r port macro lanes; do
    ...
    # <name>_<SOC_PORT_NAME>, then alternates, then the same with trailing
    # digits stripped -- which is a property applying to every port of that
    # type. A key of <name>_<logical> is not in that chain at all, so a table
    # written that way is loaded, counted, and never consulted per port.
    #
    # That is not a hypothetical: it produced two ports with different
