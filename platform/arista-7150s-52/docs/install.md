...
- [x] **the image builds.** Minimal x86_64, 2026-09-23: rootfs 43.4 MiB, kernel
      13.6 MiB, installable SWI 14.8 MiB, RAM-boot SWI 58 MiB. `nosd-fm6000`
      is in it, so the `asic: fm6000` axis resolves to its provider
- [x] **a RAM-bootable SWI exists.** The aboot backend had no `Netboot`, so
      `--ram-boot` produced a kernel and an initramfs and nothing that Aboot
      could be pointed at. It does now, and the bundle ships a README
- [ ] set `boot_mib` / `slot_mib` / `data_mib`. The default layout put a
      43.4 MiB image in a 96 MiB slot, and 511 MB is free, so two slots and a
      data partition fit comfortably — but state them from what was measured
- [ ] **RAM-boot it and see whether it comes up.** Nothing is written, so this
      is the cheap half of M0; the reboot question is the expensive half
